...
std::pair<std::string, std::string> RDMAManager::parse_ip_port(const std::string &ip_port) {
    std::string ip, port;
...
void RDMAManager::connect_to_all_servers() {
    ...
    if (my_id != num_servers - 1) {
        send_connections(my_id + 1, num_servers - my_id - 1,
...
RDMAManager::get_client_ep_and_info(struct ibv_qp_init_attr attr, void *my_info,
    ...
    return std::make_pair(new VerbsEP(id, attr, recv_batch, recv_with_data), (connect_info*)buf);
}



void RDMAManager::receive_connections(int starting_client_id, int num_clients, int max_send_size,
                                      int max_recv_size) {
    struct ibv_qp_init_attr attr =
        prepare_qp(this->getPD(), max_send_size, max_recv_size, false);

    // 1. Allocate 64 bytes per client (CPU cache-line aligned) to prevent false sharing
    ...
    for (uint32_t i = 0; i < num_clients; i++) {
        ...
        struct ibv_mr *mr = ibv_reg_mr(this->getPD(), mem, RING_BUFFER_SIZE * 2,
                                       IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE |
                                       IBV_ACCESS_REMOTE_READ);

        printf("MagicBuffer at %p and it repeats at %p\n", mem, mem + RING_BUFFER_SIZE);
...
RDMAManager::get_server_ep_and_info(int server_id, struct ibv_qp_init_attr attr,
    ...
    struct rdma_cm_id * client_cm_id = send_connect_request(server_ip.c_str(), server_port.c_str());
    VerbsEP* ep = nullptr;
...
void RDMAManager::send_connections(int starting_server_id,
                                   int num_servers, int max_send_size,
                                   int max_recv_size) {
    struct ibv_qp_init_attr attr =
        prepare_qp(this->getPD(), max_send_size, max_recv_size, false);
    ...
    memset(mem2, 0, mem2_size);
    struct ibv_mr *mr2 =
        ibv_reg_mr(this->getPD(), mem2, 4096,
                   IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE |
                   IBV_ACCESS_REMOTE_READ | IBV_ACCESS_REMOTE_ATOMIC);
...
void RDMAManager::cleanup() {
    ...
    for (int i = 0; i < num_clients; i++) {
        ...
        // we don't have a completion channel and just polls from the ring buffer so don't need to destroy
    }
}
