...
public:
  ...
  StateSendServer(const std::string &communicator_json,
                  ...
                  uint32_t num_search_threads, bool use_mem_index, DistributedSearchMode dist_search_mode,bool tags, uint64_t batch_size, bool enable_locs) {
    ...
    ssd_partition_index = std::make_unique<SSDPartitionIndex<T>>(
        m, my_partition_id, num_partitions, num_search_threads, reader,
								 communicator, dist_search_mode, tags, nullptr, batch_size, enable_locs);
    int res =
  ...
  if (type == "uint8") {
    auto server = std::make_unique<StateSendServer<uint8_t>>(
        ...
        my_partition_id, num_partitions, num_search_threads, use_mem_index,
							     dist_search_mode, use_tags, batch_size, enable_locs);
