...
namespace pipeann {
  ...
  void add_new_file_to_single_index(std::string index_file, std::string new_file) {
    ...
    pipeann::save_bin<uint64_t>(index_file, new_meta.data(), new_meta.size(), 1, 0);
  }
  ...
  bool build_disk_index(const char *dataPath, const char *indexFilePath, const char *indexBuildParameters,
                        pipeann::Metric _compareMetric, bool single_file_index, const char *tag_file, bool remove_mem_index) {
  ...
  template bool build_disk_index<int8_t, uint32_t>(
      ...
      const char *indexBuildParameters, pipeann::Metric _compareMetric,
						   bool singleFileIndex, const char *tag_file, bool remove_mem_index);
