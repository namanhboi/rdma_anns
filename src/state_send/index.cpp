...
namespace pipeann {
  ...
  template<typename T, typename TagT>
  uint64_t Index<T, TagT>::save_tags(std::string tags_file, size_t offset) {
    if (!_enable_tags) {
  ...
  template<typename T, typename TagT>
  uint64_t Index<T, TagT>::save_data(std::string data_file, size_t offset) {
  ...
  uint64_t Index<T, TagT>::save_graph(std::string graph_file, size_t offset) {
    ...
    unsigned epuint32_t = _ep;
    out.write((char *) &epuint32_t, sizeof(unsigned));
...
#pragma omp parallel for schedule(dynamic)
    for (int64_t block = 0; block < total_blocks; ++block) {
      ...
      for (int64_t i = block * block_size;
        ...
        if ((_delete_set.find((uint32_t) i) == _delete_set.end()) && (_empty_slots.find((uint32_t) i) == _empty_slots.end())) {
          candidate_set.clear();
