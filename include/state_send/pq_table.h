...
namespace pipeann {
  ...
  class FixedChunkPQTable {
   ...
   public:
    ...
    void compute_distances_alltoall(const uint8_t *comp_src, const uint8_t *comp_dsts, float *dists, const uint32_t count) {
      std::memset(dists, 0, count * sizeof(float));
      for (uint64_t i = 0; i < count; i++) {
        for (uint64_t c = 0; c < n_chunks; c++) {
          dists[i] +=
              all_to_all_dists[(uint64_t) comp_src[c] * 256 * n_chunks + (uint64_t) comp_dsts[i * n_chunks + c] * n_chunks + c];
        }
      }
    }
