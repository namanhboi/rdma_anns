...
uint64_t SendAckAsync(uint32_t freed_bytes) {
    ...
    // Grab a hardware slot from our local memory pool
    uint32_t slot_index = next_id_ % 128;
    uint64_t slot_base_addr = local_mem + (slot_index * 16);

    // =================================================================
    // 1. MEMORY PACKING (11 Contiguous Bytes)
    // =================================================================
    ...
    *(volatile MAGIC_BYTE_T*)(void*)(slot_base_addr + 10) = 1;
    ...
    wr.num_sge = 1; // Explicitly tell the NIC to only read 1 element

    // =================================================================
    // 3. HARDWARE HANDOFF & NON-BLOCKING BACKPRESSURE
    // =================================================================
    uint32_t ring_allocation = 11 - sizeof(MAGIC_BYTE_T);
    uint64_t rem_addr = remote_buffer->GetWriteAddr(ring_allocation);

    if (rem_addr == 0) {
        // REMOTE BUFFER FULL: Return -1 so send_loop can try again
        return (uint64_t)-1;
    }

    // THE ALIGNMENT FIX: +1 Byte Shift
    wr.wr.rdma.remote_addr = rem_addr + 1;
    wr.wr_id = next_id_;

    // Capture the REAL error code, not the fake errno!
