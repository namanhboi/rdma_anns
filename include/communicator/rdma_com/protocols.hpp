...
public:
  ...
  bool AckSentBytes(uint32_t bytes) { return true; }
...
public:
  ReceiveReceiver(VerbsEP *ep, Buffer *local_buffer, uint32_t size,
                  uint32_t max_recv_size)
      : ep(ep), local_buffer(local_buffer), lkey(local_buffer->GetKey()),
        size(size) {
    uint32_t i = 0;
...
public:
  SharedSendConnection(VerbsEP *ep)
      : ep(ep) {
          // empty
        };
  ...
  bool TestSend(uint64_t id) {
    ...
    for (int i = 0; i < ret; i++) {
      if (wcs[i].status != IBV_WC_SUCCESS)
        printf("Failed recv request %d \n", wcs[i].status);
      last_wrid += wcs[i].imm_data;
    }
    if (ret > 0)
      ep->post_empty_recvs((uint32_t)ret);
...
public:
  SharedReceiveReceiver(std::vector<VerbsEP *> &eps, Buffer *local_buffer,
                        uint32_t size, uint32_t max_recv_size)
      : eps(eps), local_buffer(local_buffer), lkey(local_buffer->GetKey()),
        size(size), qpn_offset(eps[0]->qp->qp_num) {
  ...
  int Receive(std::vector<Region> &v) {
    ...
    for (int i = 0; i < ret; i++) {
      if (wcs[i].opcode == IBV_WC_RECV) {
        uint32_t length = wcs[i].byte_len;
        uint32_t conid = (wcs[i].qp_num - qpn_offset) /
                         2; // connection id.todo. /2 for debugging on one node
...
public:
  CircularConnectionMailBox(VerbsEP *ep, RemoteBuffer *remote_buffer,
                            uint64_t rem_mailbox, uint32_t rem_mailbox_rkey,
                            uint64_t local_mem, uint32_t local_mem_lkey)
      : ep(ep), remote_buffer(remote_buffer), rem_mailbox(rem_mailbox),
        rem_mailbox_rkey(rem_mailbox_rkey), with_sges(ep->GetMaxSendSge() >= 2),
        local_mem(local_mem) {
    sges[0] = {local_mem, sizeof(LEN_BYTE_T),
               local_mem_lkey}; // TODO. In theory can be subject to race
                                // conditions. Should use outstanding count
    sges[1] = {0, 0, 0};
  ...
  void PrintInfo() {
    ...
    printf("Way of informing the receiver: RDMA Write to remote mailbox "
           "message ID.\n");
    printf("Prefix: Yes - Message Length.\n");
    printf("Suffix: No.\n");
    if (with_sges)
      printf("Sges: Yes - 2 to send prefix\n");
    printf("Downsides: 2 writes per send message\n");
    printf("-----------------------------------------\n");
  }

  uint32_t ReqSize(Region &region) {
    return region.length + sizeof(LEN_BYTE_T);
  }
  ...
  uint64_t SendAsync(Region &region) {
    ...
    if (with_sges) {
      *(volatile LEN_BYTE_T *)(void *)(local_mem) = (LEN_BYTE_T)region.length;
      ...
      // we prepend length
      *(LEN_BYTE_T *)(region.addr - sizeof(LEN_BYTE_T)) =
          (LEN_BYTE_T)region.length; // write len at the begin
    ...
    sges[2] = {(uint64_t)(void *)&mail_box_val, sizeof(MAGIC_BYTE_T),
               0}; // 0 as send inline
    wrs[0].wr_id = next_id_;
    wrs[0].wr.rdma.remote_addr = rem_addr;

    /*   printf("Send Async to %lx %u and to mailbox %lx %u\n",
         wrs[0].wr.rdma.remote_addr, wrs[0].wr.rdma.rkey,
       wrs[1].wr.rdma.remote_addr, wrs[1].wr.rdma.rkey );
...
public:
  CircularConnectionMagicbyte(VerbsEP *ep, RemoteBuffer *remote_buffer,
                              uint64_t local_mem, uint32_t local_mem_lkey)
  ...
  void PrintInfo() {
    ...
    printf("Type: P2P.\n");
    printf("Used OPs: RDMA Write\n");
    printf("Way of informing the receiver: the receiver polls len and magic "
           "byte.\n");
    printf("Prefix: Yes - Message Length.\n");
  ...
  uint64_t SendAsync(Region &region) {
    ...
    uint32_t real_length =
        region.length + sizeof(LEN_BYTE_T) + sizeof(MAGIC_BYTE_T);
    ...
    if (with_sges) {
      ...
      *(LEN_BYTE_T *)(region.addr - sizeof(LEN_BYTE_T)) =
          (LEN_BYTE_T)region.length; // write len at the begin
      *(MAGIC_BYTE_T *)(region.addr + region.length) =
          (MAGIC_BYTE_T)1; // write 1 at the end
      send_addr -= sizeof(LEN_BYTE_T);
    }
...
public:
  CircularConnectionNotify(VerbsEP *ep, RemoteBuffer *remote_buffer)
      : ep(ep), remote_buffer(remote_buffer) {
          // empty
        };
...
public:
  CircularConnectionReverse(VerbsEP *ep, RemoteBuffer *remote_buffer,
    ...
    remote_buffer->GetWriteAddr(sizeof(MAGIC_BYTE_T) + sizeof(uint8_t));
  }
  ...
  uint64_t SendAsync(uint8_t opcode, Region &region) {
    struct ibv_send_wr *bad_wr;
    ...
    uint32_t prefix_len = sizeof(MAGIC_BYTE_T) + sizeof(uint8_t);
    uint32_t suffix_len = sizeof(LEN_BYTE_T) + sizeof(MAGIC_BYTE_T);
    uint32_t real_length = prefix_len + region.length + suffix_len;
    ...
    *(volatile uint8_t *)(void *)(slot_base_addr + sizeof(MAGIC_BYTE_T)) =
...
public:
  CircularMailBoxReceiver(Buffer *local_buffer, uint64_t mailbox_addr,
                          uint32_t mailbox_rkey)
      : local_buffer(local_buffer), mailbox_addr(mailbox_addr),
        mailbox_rkey(mailbox_rkey),
        pending((volatile MAGIC_BYTE_T *)(void *)mailbox_addr) {
  ...
  int Receive(std::vector<Region> &v) {
    ...
    while (processed < pending_copy) {
      ...
      //     printf("length %lu\n",length);
      char *whole_message =
          local_buffer->Read(((uint32_t)length) + sizeof(LEN_BYTE_T));
      uint32_t lkey = 0; // I do not need this
      v.push_back(
          {0, whole_message + sizeof(LEN_BYTE_T), (uint32_t)length, lkey});
      processed++;
      c++;
    }
...
public:
  ...
  int Receive(std::vector<Region> &v) {
    ...
    while (length != 0) { // we received a message
      ...
      *(volatile LEN_BYTE_T *)local_buffer->GetReadPtr() =
          (LEN_BYTE_T)0; // zero the length. otherwise we can read length of an
                         // old message.
      char *whole_message = local_buffer->Read(
          (uint32_t)length + sizeof(LEN_BYTE_T) + sizeof(MAGIC_BYTE_T));
      uint32_t lkey = 0; // I do not need this
      v.push_back(
          {0, whole_message + sizeof(LEN_BYTE_T), (uint32_t)length, lkey});
      MAGIC_BYTE_T completion = 0;
      do {
        completion = *(volatile MAGIC_BYTE_T *)(whole_message +
                                                sizeof(LEN_BYTE_T) + length);
      } while (completion == 0);
      c++;
      //    printf("done %lu \n",length);
...
public:
  ...
  void FreeReceive(Region &region) { local_buffer->Free(region.length); }
...
public:
   ...
   */
  int Receive(std::vector<Region> &v) {
    MAGIC_BYTE_T completion =
        *(volatile MAGIC_BYTE_T *)local_buffer->GetReadPtr();
    int c = 0;

    while (completion) { // we received a message
      LEN_BYTE_T length =
          *(volatile LEN_BYTE_T *)local_buffer->Read(sizeof(LEN_BYTE_T));
      // printf("Get a new len %lu \n",length);

      char *whole_message = local_buffer->Read(
          (uint32_t)length +
          sizeof(MAGIC_BYTE_T)); // we read the byte with zero.
      uint32_t lkey = 0;         // I do not need this
      v.push_back(
          {0, whole_message + sizeof(MAGIC_BYTE_T), (uint32_t)length, lkey});
      c++;
      //   printf("done %lu \n",length);
...
class SharedCircularConnectionMailbox : public SendCommunicator {
  const uint8_t total_faa_slots =
      16; // usually we cannot have more than 16 outstanding atomics
  ...
  const uint64_t
      local_mem; // read here and faa here. should be at least 16 bytes
...
public:
  SharedCircularConnectionMailbox(VerbsEP *ep, MagicRemoteBuffer *remote_buffer,
                                  uint64_t rem_head, uint32_t rem_head_rkey,
                                  uint64_t rem_win, uint32_t rem_win_rkey,
                                  uint64_t local_mem, uint32_t local_mem_lkey)
      : ep(ep), remote_buffer(remote_buffer), rem_head(rem_head),
        rem_head_rkey(rem_head_rkey), rem_win(rem_win),
        rem_win_rkey(rem_win_rkey), local_mem(local_mem),
        local_mem_lkey(local_mem_lkey),
        pending_val((volatile uint64_t *)(void *)(local_mem +
                                                  (total_faa_slots + 1) * 8)),
        with_sges(ep->GetMaxSendSge() >= 3) {

    for (uint8_t i = 0; i < total_faa_slots;
         i++) { // local faa slots to have parallelization
      free_local_slots.push_back(i);
    }

    sges[0] = {local_mem, 8, local_mem_lkey}; // fetch here
    sges[1] = {local_mem + (total_faa_slots + 1) * 8, 8,
               local_mem_lkey}; // always read here
    ...
    wrs[1].opcode =
        IBV_WR_RDMA_READ; // To fetch head.  // it may target device memory!
    ...
    sges[2] = {local_mem + (total_faa_slots + 2) * 8, 8,
               local_mem_lkey}; // fetch here
    sges[4] = {local_mem + (total_faa_slots + 3) * 8, 8,
               local_mem_lkey}; // always read here

    *(volatile MAGIC_BYTE_T *)(void *)(local_mem + (total_faa_slots + 3) * 8) =
        (MAGIC_BYTE_T)1;

    wrs[2].opcode =
        IBV_WR_RDMA_WRITE; // TODO. SHould we try target dev memory here?
  ...
  void PrintInfo() {
    ...
    printf(
        "Comment: One should call test send to fetch head and trigger send\n");
    printf("-----------------------------------------\n");
...
private:
  inline void send(uint64_t fetched, Region *region) {
    ...
    uint32_t offset =
        remote_buffer->GetOffset(remote_buffer->GetBaseAddr() + fetched);
    ...
    if (with_sges) {
      *(volatile LEN_BYTE_T *)(void *)(local_mem +
                                       ((total_faa_slots + 2) * 8)) =
          (LEN_BYTE_T)region->length;
      real_length = region->length;
      send_addr = ((uint64_t)(void *)region->addr);
    } else {
      *(volatile LEN_BYTE_T *)(region->addr - sizeof(LEN_BYTE_T)) =
          (LEN_BYTE_T)region->length; // write len at the begin
      *(volatile MAGIC_BYTE_T *)(region->addr + region->length) =
          (MAGIC_BYTE_T)1; // write 1 at the end
    }
  ...
  inline bool try_send_delayed() {
    ...
    while (!delayed.empty()) {
      ...
      uint32_t real_length =
          val.second->length + sizeof(LEN_BYTE_T) + sizeof(MAGIC_BYTE_T);
...
public:
  SharedCircularConnectionNotify(VerbsEP *ep, MagicRemoteBuffer *remote_buffer,
                                 uint64_t rem_head, uint32_t rem_head_rkey,
                                 uint64_t rem_win, uint32_t rem_win_rkey,
                                 uint64_t local_mem, uint32_t local_mem_lkey)
      : ep(ep), remote_buffer(remote_buffer), rem_head(rem_head),
    ...
    wrs[1].opcode = IBV_WR_RDMA_READ; // To fetch head. TODO. SHould we try
                                      // target dev memory here?
  ...
  void PrintInfo() {
    ...
    printf("Role: Sender\n");
    printf("Type: Many to One.\n");
    printf("Used OPs: RDMA Write with IMM, RDMA Atomic, RDMA Read to fetch "
           "head\n");
    printf("Way of informing the receiver: offset in the IMM data\n");
    printf("Prefix: No\n");
    printf("Suffix: No\n");
    printf("Downsides: Use of IMM\n");
    printf("Comment: I can fetch multiple faas to have many outstanding. Head "
           "is shared.\n");
    printf("-----------------------------------------\n");
...
private:
  inline void send(uint64_t fetched, Region *region) {
    ...
    uint32_t real_length = region->length;
    struct ibv_sge sge = {send_addr, real_length, region->lkey};
    if (ep->write_imm(&sge, 1, remote_buffer->GetKey(), rem_addr, next_id_,
                      offset)) {
      printf("Failed to send\n");
      exit(1);
    }
...
public:
  SharedCircularMagicbyteReceiver(MagicRingBuffer *local_buffer, void *mailbox,
                                  void *faaaddr, struct ibv_dm *dm)
      : local_buffer(local_buffer), head((volatile uint64_t *)mailbox),
        faa((volatile uint64_t *)faaaddr), dm(dm) {
  ...
  void FreeReceive(Region &region) {
    // printf("free receive \n");
    lhead = local_buffer->Free(region.length + sizeof(LEN_BYTE_T) +
                               sizeof(MAGIC_BYTE_T));
    if (head != NULL) {
      //     printf("write %lu \n",lhead);
      *head = lhead;
...
public:
  SharedCircularNotifyReceiver(std::vector<VerbsEP *> &eps,
                               MagicRingBuffer *local_buffer, void *mailbox,
                               void *faaaddr, struct ibv_dm *dm)
      : recv_cq(eps[0]->qp->recv_cq), eps(eps), local_buffer(local_buffer),
        head((volatile uint64_t *)mailbox), faa((volatile uint64_t *)faaaddr),
        dm(dm) {
    lhead = local_buffer->Free(0); // it will set the value of buffer length.
    if (lhead == 0) {
      printf("head cannot be zero at the beginning!\n");
  ...
  int Receive(std::vector<Region> &v) {
    ...
    for (int i = 0; i < ret; i++) {
      if (wcs[i].opcode == IBV_WC_RECV_RDMA_WITH_IMM) {
        ...
        v.push_back({0, whole_message, (uint32_t)length, lkey});
        c++;
        eps[wcs[i].wr_id]->post_empty_recvs(
            1); // if shared it will use the first eps.
      }
    }
    return c;
  }
...
public:
  ReadCircularConnectionNotify(std::vector<VerbsEP *> &eps,
                               MagicRemoteBuffer *local_buffer,
                               uint64_t tail_ptr)
      : eps(eps), local_buffer(local_buffer),
        tails((volatile uint64_t *)(void *)tail_ptr),
        num_of_clients(eps.size()) {
  ...
  uint64_t SendAsync(Region &region) {
    ...
    char *whole_message =
        (char *)(void *)(local_buffer->GetWriteAddr(real_length));
...
private:
  ...
  inline void try_send_delayed() {
    ...
    while (!delayed.empty()) {
      ...
      update_tail();
      char *whole_message =
          (char *)(void *)(local_buffer->GetWriteAddr(region->length));
      if (whole_message == NULL) {
        break;
      }
      // copy to my memory
...
private:
  inline void update_tail() {
    volatile uint64_t *new_tail =
        std::min_element(tails, tails + num_of_clients);
    local_buffer->UpdateHead(*new_tail);
    replicated_tail = *new_tail;
  }
};

// sender locally copies data to buffer. It can fail as readers need to progress
// it.
...
public:
  ReadCircularConnectionMagicByte(uint32_t num_of_clients,
                                  MagicRemoteBuffer *local_buffer,
                                  uint64_t local_tail_ptr, uint64_t tail_ptrs,
                                  struct ibv_dm *dm)
      : local_buffer(local_buffer),
        tails((volatile uint64_t *)(void *)(tail_ptrs)),
        num_of_clients(num_of_clients),
        current_local_tail((volatile uint64_t *)(void *)(local_tail_ptr)),
        dm(dm) {
  ...
  void PrintInfo() {
    ...
    printf("Type: One to Many.\n");
    printf("Used OPs: No\n");
    printf("Way of informing the receiver: write local tail that is RDMA "
           "readable\n");
    printf("Way of informing the receiver 2: writes len after writing data. So "
           "magic byte.\n");
    printf("Prefix: Yes - length.\n");
    printf("Suffix: No.\n");
  ...
  uint64_t SendAsync(Region &region) {
    ...
    *(volatile LEN_BYTE_T *)(whole_message + real_length) =
        (LEN_BYTE_T)0; // write 0 at the end
    *(volatile LEN_BYTE_T *)(whole_message) =
        (LEN_BYTE_T)region.length; // write len at the begin
...
public:
  ReadCircularNotifyReceiver(VerbsEP *ep, MagicRingBuffer *remote_buffer,
                             const uint64_t rem_tail,
                             const uint32_t rem_tail_rkey)
      : ep(ep), remote_buffer(remote_buffer), rem_tail(rem_tail),
        rem_tail_rkey(rem_tail_rkey) {
  ...
  void PrintInfo() {
    ...
    printf("Role: Receiver\n");
    printf("Type: One to Many.\n");
    printf("Used OPs: Post Recv (get notifications), Read (fetch data), and "
           "Write (to notify progress)\n");
    printf("Passive: no\n");
    printf("-----------------------------------------\n");
  }
  ...
  int Receive(std::vector<Region> &v) {
    ...
    for (int i = 0; i < ret; i++) {
      if (wcs[i].opcode == IBV_WC_RECV_RDMA_WITH_IMM) {
        ...
        v.push_back(
            {1, whole_message, (uint32_t)length,
             remote_buffer->GetKey()}); // it is pointer to remote memory
        c++;
  ...
  void FreeReceive(Region &region) { remote_buffer->Free(region.length); }
...
public:
  ReadCircularMagicByteReceiver(VerbsEP *ep, MagicRingBuffer *remote_buffer,
                                const uint64_t rem_tail,
                                const uint32_t rem_tail_rkey,
                                uint64_t local_mem, uint32_t local_mem_lkey,
                                uint32_t prefetch_size)
      : ep(ep), remote_buffer(remote_buffer), rem_tail(rem_tail),
        rem_tail_rkey(rem_tail_rkey), lkey_of_prefetch(local_mem_lkey),
  ...
  void PrintInfo() {
    ...
    printf("Role: Receiver\n");
    printf("Type: One to Many.\n");
    printf("Used OPs: Post Recv (get notifications), Read (prefetch len and "
           "fetch data), and Write (to notify progress)\n");
    printf("Passive: no\n");
    printf("Add-ons: can prefetch data\n");
    printf("Comment: it expects in order processing from clients. Otherwise I "
           "need to implement out-of-order free\n");
    printf("-----------------------------------------\n");
  }
      ...
      if (wcs[i].opcode == IBV_WC_RDMA_READ) {
        ...
        if (!addr) {
          ...
          while (offset <= prefetch_size) {
            if (offset + sizeof(LEN_BYTE_T) >
                prefetch_size) { // we do not have length in the message.
            ...
            if (length == 0) {
              ...
              remote_buffer->Read(
                  length + sizeof(LEN_BYTE_T)); // we can move the pointer.
              // uint64_t raddr = (uint64_t)(void*)remote_buffer->GetReadPtr();
              // printf("I will try to read next from %lx\n", raddr);
              v.push_back({1, part_message + offset + sizeof(LEN_BYTE_T),
                           (uint32_t)length,
                           slot}); // slot will be used only in the last partial
  ...
  bool Receive(Region &s, Region &d) { // receive from source to destination
    ...
    uint32_t fetched = (temp_recv_regions[slot] + prefetch_size) -
                       ((uint64_t)(void *)s.addr); // only payload
    ...
    if (s.length > fetched) {
      ...
      struct ibv_sge sge = {(uint64_t)(void *)d.addr + fetched,
                            need_to_fetch_more, d.lkey};
...
public:
  ReadCircularMailboxReceiver(VerbsEP *ep, MagicRingBuffer *remote_buffer,
                              const uint64_t rem_tail,
                              const uint32_t rem_tail_rkey, uint64_t rem_head,
                              uint32_t rem_head_rkey, uint64_t local_mem,
                              uint32_t local_mem_lkey, bool in_order)
      : ep(ep), remote_buffer(remote_buffer), rem_tail(rem_tail),
        rem_tail_rkey(rem_tail_rkey), rem_head(rem_head),
        rem_head_rkey(rem_head_rkey), local_mem(local_mem),
        local_mem_lkey(local_mem_lkey),
        remote_head((volatile uint64_t *)(void *)local_mem),
        in_order(in_order) {
  ...
  void PrintInfo() {
    ...
    printf("Role: Receiver\n");
    printf("Type: One to Many.\n");
    printf("Used OPs:  Read (fetch tail and fetch data), and Write (to notify "
           "progress)\n");
    printf("Passive: no\n");
    printf("Add-ons:  ordered and not ordered recv.\n");
    printf(
        "Problems:  The receiver does not know the size of fetched records.\n");
    printf("-----------------------------------------\n");
  }
  ...
  int Receive(std::vector<Region> &v) {
        ...
        if (wcs[i].wr_id == 0) { // updated head
          ...
          while (offset < total_length) {
            ...
            if (offset + sizeof(LEN_BYTE_T) + length > total_length) {
              ...
              if (length + sizeof(LEN_BYTE_T) !=
                  len_region) { // we can strip first entry

                //   printf(">>>>>>>>>>>>>>>>..Push partial\n");
                v.push_back({1, (char *)(void *)rem_addr,
                             (uint32_t)(length + sizeof(LEN_BYTE_T)), 0});
              ...
              uint32_t lkey =
                  in_order ? 0
                           : remote_buffer->GetOffset((char *)(void *)raddr);

              // printf(">>>>>>>>>>>>>>>>..Push finished\n");
              v.push_back({0,
                           (char *)(void *)laddr + offset + sizeof(LEN_BYTE_T),
    ...
    if (!has_pending_read) { // fetch remote head
      struct ibv_sge sge = {(uint64_t)(void *)local_mem, sizeof(uint64_t),
                            local_mem_lkey};
    ...
    if (can_fetch) {
      ...
      if (!delayed.empty() && in_order) {
        //        printf("I could read. but because of in-order requirement I
        //        delay it.\n");
  ...
  bool Receive(Region &s, Region &d) { // receive from source to destination
    ...
    if (partial_read) {
      ...
      delayed.push_back(
          {(uint64_t)(void *)s.addr + to_fetch, s.length - to_fetch});
    }
  ...
  void FreeReceive(Region &region) { // we free full reads only
    ...
    if (in_order) {
      ...
      remote_buffer->FreeOrdered(remote_buffer->GetReadPtr(region.lkey),
                                 region.length + sizeof(LEN_BYTE_T));
    }
  }
