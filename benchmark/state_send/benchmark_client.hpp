...
public:
  ...
  virtual void query(const T *query, const uint64_t k_search,uint32_t mem_L, const uint64_t l_search, const uint32_t beam_width) = 0;
...
class LocalBenchmarkClient : public BenchmarkClient<T, TagT> {
  class SSDPartitionSearchThread {
    std::thread real_thread;
    bool running = false;
    void main_loop();

    // moodycamel::BlockingConcurrentQueue<std::shared_ptr<>
    
  public:
    SSDPartitionSearchThread();
    void start();
    void signal_stop();
    void join();
  };
...
public:
  ...
  void query(const T *query, const uint64_t k_search,uint32_t mem_L, const uint64_t l_search, const uint32_t beam_width) override;
