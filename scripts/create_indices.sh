...
# this file will create the indices for both the scatter gather and state send  # approach from the partition file and put them in the specifided folders
...
"${WORKDIR}/build/src/state_send/create_base_file_from_loc_file ${DATA_TYPE} ${BASE_FILE} ${PARTITION_FILE} ${PARTITION_BASE_FILE_PATH}"

# now we need to create the pq file
"${WORKDIR}/build/src/state_send/create_pq_data ${DATA_TYPE} ${PARTITION_BASE_FILE_PATH} ${SCATTER_GATHER_INDEX_PREFIX} ${METRIC} ${SCATTER_GATHER_NUM_PQ_CHUNKS}"
...
"${WORKDIR}/build/src/state_send/gen_random_slice ${PARTITION_BASE_FILE_PATH} ${SCATTER_GATHER_SLICE_PREFIX} ${MEM_INDEX_SAMPLING_RATE}"

"${WORKDIR}/build/src/state_send/build_memory_index ${DATA_TYPE} ${SCATTER_GATHER_SLICE_PREFIX}_data.bin ${SCATTER_GATHER_SLICE_PREFIX}_ids.bin ${MEM_INDEX_R} ${MEM_INDEX_L} ${MEM_INDEX_ALPHA} ${SCATTER_GATHER_INDEX_PREFIX}_mem.index ${NUM_THREADS} ${METRIC}"


# now we actually create the disk index for scatter gather
RAM_BUDGET=96
"${WORKDIR}/build/src/state_send/build_disk_index ${DATA_TYPE} ${PARTITION_BASE_FILE_PATH} ${SCATTER_GATHER_INDEX_PREFIX} ${SCATTER_GATHER_L} ${SCATTER_GATHER_L} 3.3 ${RAM_BUDGET} ${NUM_THREADS} ${METRIC} 0"
