...
if [[ $MODE == "local" ]]; then 
    ...
    MEDOID_FILE="${ANNGRAHPS_PREFIX}/${DATASET_NAME}/${DATASET_SIZE}/clusters_${NUM_SERVERS}/medoids.bin"
fi
...
if [[ $DIST_SEARCH_MODE != "SCATTER_GATHER_TOP_N" ]]; then
    ...
    EXPERIMENT_NAME=${DIST_SEARCH_MODE}${TOP_N}_${MODE}_${DATASET_NAME}_${DATASET_SIZE}_${NUM_SERVERS}_${COUNTER_SLEEP_MS}_MS_NUM_SEARCH_THREADS_${NUM_SEARCH_THREADS}_MAX_BATCH_SIZE_${MAX_BATCH_SIZE}_K_${K_VALUE}_OVERLAP_${OVERLAP}_BEAMWIDTH_${BEAM_WIDTH}
fi
