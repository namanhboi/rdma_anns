...
std::vector<send_request_t> generate_send_request_batch(int num_send_requests, int num_clusters) {
  ...
  std::vector<send_request_t> send_requests;

  for (auto i = 0; i < num_send_requests; i++) {
    send_requests.push_back({cluster_id_gen(gen), uint64_t_gen(gen)});
  }
...
void benchmark(const std::string &query_file, uint32_t batch_min_size,
               uint32_t batch_max_size, uint32_t batch_time_us,
               uint32_t num_result_threads, uint32_t send_rate, uint32_t K,
               uint32_t L) {
  ...
  client.setup(batch_min_size, batch_max_size, batch_time_us,
               num_result_threads);
